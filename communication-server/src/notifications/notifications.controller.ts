...
export class NotificationsController {
  ...
  async handleCalendarUpdate(
    @Body() payload: { userId: number; message: string; eventsCount?: number },
  ) {
    console.log(
      `Received calendar update for user ${payload.userId}: ${payload.message}`,
    );
    // 특정 사용자에게 캘린더 업데이트 이벤트 전송
    this.communicationGateway.sendCalendarUpdateToUser(
