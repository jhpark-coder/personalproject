...
export class SmsService implements OnModuleInit {
  ...
  private initializeTwilioClient() {
    try {
      ...
      console.log('모든 TWILIO 관련 환경 변수:', Object.keys(process.env).filter(key => key.includes('TWILIO')));
      ...
      console.log('Twilio Account SID:', accountSid ? `${accountSid.substring(0, 10)}...` : 'undefined');
      console.log('Twilio Auth Token:', authToken ? '***설정됨***' : 'undefined');
      ...
      if (accountSid && authToken) {
        ...
        console.error('Please check your .env.development file and ensure TWILIO_* variables are set');
      }
    } catch (error) {
   ...
   * 운동 추천 SMS 발송
   */
  async sendWorkoutRecommendation(to: string, workout: string): Promise<boolean> {
  ...
  async requestOtp(phone: string): Promise<{ success: boolean; message?: string }> {
    try {
      ...
      if (!rateLimitCheck.allowed) {
        const remainingMinutes = Math.ceil((rateLimitCheck.remainingTime || 0) / 60);
        return { 
          success: false, 
      ...
      // 레이트 리밋 증가
      await this.redisService.incrementRateLimit(phone, otpConfig.rateLimitWindow);
  ...
  async verifyOtp(phone: string, code: string): Promise<{ success: boolean; message?: string }> {
    try {
      ...
      return { success: false, message: '인증 코드 확인 중 오류가 발생했습니다.' };
    }
  }
