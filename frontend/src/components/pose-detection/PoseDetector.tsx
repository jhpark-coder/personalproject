...
const PoseDetector: React.FC = () => {
  ...
  // 플랭크 판정(정적)
  const analyzePlankWithJudge = (lm: any[]): ExerciseAnalysis => {
    const shL = lm[LEFT_SHOULDER], shR = lm[RIGHT_SHOULDER];
    const elL = lm[LEFT_ELBOW], elR = lm[RIGHT_ELBOW];
    const hipL = lm[LEFT_HIP], hipR = lm[RIGHT_HIP];
  ...
  // 카프 레이즈(발끝-발목 y차 증가)
  const analyzeCalfRaiseWithCount = (lm: any[]): ExerciseAnalysis => {
    const ankleL = lm[LEFT_ANKLE], ankleR = lm[RIGHT_ANKLE];
  ...
  const baseNA = (type: string): ExerciseAnalysis => ({ exerciseType: type, currentCount: stateRef.current.count, isCorrectForm: false, feedback: '랜드마크 부족', confidence: 0 });
