...
const getUserIdFromToken = (): number | null => {
  try {
    ...
    // 백엔드에서는 userId를 sub 필드에 저장하므로 sub를 우선 확인
    // 만약 sub가 없다면 userId 필드도 확인
    const userId = payload.sub || payload.userId || null;
    
    if (userId) {
      console.log('JWT 토큰에서 userId 추출 성공:', userId);
      return parseInt(userId.toString());
    }
    
    console.warn('JWT 토큰에서 userId를 찾을 수 없습니다. 페이로드:', payload);
    return null;
  } catch (error) {
...
const ChatPage: React.FC<ChatPageProps> = ({ onClose, isModal = true }) => {
  ...
  useEffect(() => {
    ...
    // 토큰이 유효하지 않으면 채팅 연결하지 않음
    if (!userId) {
      console.error('유효한 토큰이 없어 채팅에 연결할 수 없습니다.');
      return;
    }
    ...
    newSocket.on('connect', () => {
      ...
      if (!userId) {
        console.error('연결 후 userId를 가져올 수 없습니다.');
        return;
      }
  ...
  const sendMessage = () => {
    ...
    const userId = getUserIdFromToken();
    if (!userId) {
      console.error('유효한 userId가 없어 메시지를 전송할 수 없습니다.');
      return;
    }
