...
const OnboardingBasicInfo: React.FC = () => {
  ...
  const handleSmsSend = async () => {
    ...
    try {
      ...
      const response = await communicationClient.post(API_ENDPOINTS.VERIFY_PHONE, { phoneNumber: formattedPhone });
      ...
      console.error('SMS 발송 실패:', errorMessage);
      showModal('SMS 발송 실패', errorMessage, 'error');
    } finally {
  ...
  const handleSmsVerify = async () => {
    ...
    try {
      const formattedPhone = formatPhoneNumber(formData.phoneNumber);
      const response = await communicationClient.post(API_ENDPOINTS.VERIFY_PHONE, { 
      ...
      const errorMessage = handleApiError(error);
      console.error('SMS 인증 실패:', errorMessage);
  ...
  const handleSmsResend = async () => {
    if (!formData.phoneNumber.trim()) {
      showModal('입력 오류', '휴대전화번호를 입력해주세요.', 'error');
      return;
    }
    ...
    try {
      ...
      if (data.success) {
        setShowSmsCodeInput(true);
        startTimer();
        showModal('재발송 완료', '인증 코드가 재발송되었습니다.', 'success');
      } else {
        showModal('재발송 실패', data.message || '인증 코드 재발송에 실패했습니다.', 'error');
      }
    } catch (error) {
      const errorMessage = handleApiError(error);
      console.error('SMS 재발송 실패:', errorMessage);
      showModal('재발송 실패', errorMessage, 'error');
    } finally {
      setIsSmsLoading(false);
    }
  };
    ...
    const loadUserData = async () => {
      try {
        ...
        if (response.data.success && response.data.user) {
          ...
          console.log('사용자 데이터가 없거나 실패:', response.data);
        }
      } catch (error) {
        const errorMessage = handleApiError(error);
        console.error('사용자 데이터 로딩 실패:', errorMessage);
        showModal('데이터 로딩 실패', errorMessage, 'error');
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!isSmsVerified) {
      newErrors.phoneNumber = '전화번호 인증을 완료해주세요';
    }
    
    if (Object.keys(newErrors).length > 0) {
      setErrors(newErrors);
      showModal('입력 오류', '필수 정보를 모두 입력해주세요.', 'error');
      return;
    }
