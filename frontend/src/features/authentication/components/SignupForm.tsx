...
// Firebase SDK import - Twilio OTP 사용으로 주석처리
// import { initializeApp } from 'firebase/app';
// import { getAuth, RecaptchaVerifier, signInWithPhoneNumber } from 'firebase/auth';
// import type { UserCredential } from 'firebase/auth';

// Firebase ConfirmationResult 타입 정의 - Twilio OTP와 호환성을 위해 유지
...
// reCAPTCHA 설정 - Twilio OTP 사용으로 주석처리
// let recaptchaVerifier: RecaptchaVerifier | null = null;

// reCAPTCHA 초기화 함수 - Twilio OTP 사용으로 주석처리
// const initializeRecaptcha = async () => { // 비동기 콜백함수 선언
//   try {
...
//     // reCAPTCHA 렌더링
//     await recaptchaVerifier.render(); // 비동기 콜백함수 렌더링
...
//TypeScript 문법으로, 타입 안정성 보장
interface FormErrors {
  email?: string; // 물음표 뜻 : 있을 수도, 없을 수도 있음
  password?: string;
  nickname?: string;
  name?: string;
  birthDate?: string;
...
const SignupForm: React.FC = () => {
  ...
  const handleExtendTime = async () => {
    ...
    try {
      ...
      // Firebase SMS 재발송 주석처리 - Twilio OTP 사용
      // reCAPTCHA 초기화
      // if (!recaptchaVerifier) {
  ...
  */
  ...
  const handleTwilioOtpRequest = async () => {
    ...
    try {
      ...
      showModal('전송 실패', errorMessage || '인증 코드 발송 중 오류가 발생했습니다.', 'error');
    } finally {
      setIsSmsLoading(false);
    }
  };
  ...
  const handleTwilioOtpVerify = async () => {
    ...
    try {
      ...
      const errorMessage = handleApiError(error);
      showModal('인증 실패', errorMessage || '인증 코드 확인 중 오류가 발생했습니다.', 'error');
    } finally {
      setIsSmsLoading(false);
    }
  };
  ...
  const handleSignup = async () => {
    ...
    if (Object.keys(formErrors).length === 0) {
      try {
        ...
        const errorMessage = handleApiError(error);
        showModal('회원가입 실패', errorMessage || '회원가입에 실패했습니다. 다시 시도해주세요.', 'error');
      }
    } else {
  ...
  */
  ...
  return (
    <div className="signup-form-container">
      ...
      {/* reCAPTCHA 컨테이너 - Twilio OTP 사용으로 주석처리 */}
