...
import { apiClient, communicationClient, handleApiError } from '@utils/axiosConfig';
...
const NotificationCenter: React.FC = () => {
  ...
  const fetchNotifications = async () => {
    ...
    try {
      setLoading(true);
      const response = await communicationClient.get(`/api/notifications/user/${user.id}`);
      ...
      setError(errorMessage || '알림을 불러오는데 실패했습니다.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const fetchUnreadCount = async () => {
    ...
    try {
      const response = await communicationClient.get(`/api/notifications/user/${user.id}/unread-count`);
      const data = response.data;
  ...
  const markAsRead = async (notificationId: string) => {
    try {
      const response = await communicationClient.put(`/api/notifications/${notificationId}/read`);
      ...
      console.error('알림 읽음 처리 실패:', handleApiError(err));
    }
  };
    ...
    try {
      ...
      await Promise.all(
        unreadNotifications.map(notification => 
          communicationClient.put(`/api/notifications/${notification._id}/read`)
        )
      );
      ...
      console.error('모든 알림 읽음 처리 실패:', handleApiError(err));
    }
  };
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(async () => {
      try {
        ...
        const res = await apiClient.get(`/api/users/search?${params.toString()}`);
  ...
  const sendNotification = async () => {
    ...
    try {
      ...
      const token = localStorage.getItem('token');
      const resUsers = await apiClient.get(`/api/users/ids`);
      if (!(resUsers.status >= 200 && resUsers.status < 300)) throw new Error('사용자 목록 조회 실패');
      const users: { id: number }[] = resUsers.data;
      ...
      const requests = payloads.map(p => apiClient.post(API_ENDPOINTS.CREATE_NOTIFICATION, p));
  ...
  const sendPersonalNotification = async () => {
    ...
    try {
      const res = await apiClient.post(API_ENDPOINTS.CREATE_NOTIFICATION, payload);
      if (!(res.status >= 200 && res.status < 300)) throw new Error('개인 발송 실패');
      setMessageInput('');
      ...
      <div className="notification-center">
        <div className="header">
          <div className="header-content content-wrapper">
            <div className="header-title">🔔 알림</div>
          </div>
        </div>
  ...
  return (
    <div className="notification-center">
      ...
      <div className="notification-inner">
        ...
        {/* 읽지 않은 알림 개수 및 모두 읽음 버튼 */}
        {unreadCount > 0 && (
          <div style={{ padding: '16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <span className="unread-badge" aria-label={`읽지 않은 알림 ${unreadCount}개`}>읽지 않은 알림: {unreadCount}개</span>
